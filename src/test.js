...
if(window.location.href.split('try=').length == 2){
	addTimer();
}
else{
...
function startCanvas(lastMillis, currentMillis){
    ...
    var fineLineInterval = 100,
        bigLineInterval = 250,
        startAngle = circleReference + lastMillis/1000 * 2 * Math.PI,
        endAngle = circleReference + currentMillis/1000 * 2 * Math.PI,
        angleDifference = endAngle - startAngle;

    var numberOfFineLines = Math.floor((currentMillis - lastMillis) / fineLineInterval),
        numberOfBigLines = Math.floor((currentMillis - lastMillis) / bigLineInterval);

    for (var i = 0; i <= numberOfFineLines; i++) {
        ctx.beginPath();
        ctx.lineWidth = 1;
        var angle = startAngle + (i * fineLineInterval / 1000 * 2 * Math.PI);
        ctx.arc(75, 75, 50, angle, angle + 0.01);
        ctx.stroke();
    }

    for (var j = 0; j <= numberOfBigLines; j++) {
        ctx.beginPath();
        ctx.lineWidth = 3;
        var angle = startAngle + (j * bigLineInterval / 1000 * 2 * Math.PI);
        ctx.arc(75, 75, 50, angle, angle + 0.02);
        ctx.stroke();
    }
}
